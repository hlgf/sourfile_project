...
class LeftNav extends Component {
  ...
  // 检查当前用户的menus
  checkdMenus = () => {
    // 首先获取当前用户的menus数据,再获取config中的menus.js中的数据
    // 在menus.js中找当前用户的menus进行对比,所以,多的里面找少的
    return menus.reduce((prev,current)=>{
      ...
      if(result){
        // 包含menu的key,所以保存的是对象,而不是key
        prev.push(current)
      }else if(current.children){
        // 说明有子级菜单,那就继续过滤
        const cMenus=current.children.filter(menu=>this.props.menus.includes(menu.key))
        if(cMenus.length){
          // 如果存在则修改当前菜单的children, current.children--->cMenu对象
          // current.children=cMenus
          // 添加到数组中
      ...
      return prev  // 返回累加数据之后的数组,prev默认就是一个数组
    },[])
  ...
  render() {
    // 检查menus
    const checkMenus = this.checkdMenus()

    // 调用方法显示菜单
    const menus = this.createMenus(checkMenus)
